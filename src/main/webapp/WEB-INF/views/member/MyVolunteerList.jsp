...
<div class="row">
	<div class="col-md-8 col-md-offset-2 fh5co-form animate-box"
		...
		<c:if test="${!empty myList}">
			<a class="more" href="createdList">more →</a>
		</c:if>
	</div>
	<div class="col-md-8 col-md-offset-2 fh5co-form animate-box"
		data-animate-effect="fadeIn">
		<div class="myTitle">■ 내가 신청한 봉사 목록 ■</div>
		<table class="table table-hover myListTbl">
			<tr>
				<th>제목</th>
				<th>봉사 타입</th>
			...
			<c:if test="${empty myList2}">
				<tr>
					<td colspan="5" align="center"><b>신청하신 봉사 활동이 없습니다.</b></td>
				</tr>
			</c:if>
			<c:forEach var="l" items="${myList2 }">
				<tr onclick="detail(${l.v_no});" class="mouse">
					<td>${l.v_title }</td>
					<td>${l.v_type }</td>
					<td><fmt:formatDate value="${l.v_date}" pattern="yyyy-MM-dd E" />
		...
		<c:if test="${!empty myList2}">
			<a class="more" href="applicationList">more →</a>
		</c:if>
	</div>
</div>
